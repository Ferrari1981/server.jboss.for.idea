...
public class TabelJsonDeserializer extends JsonMainDeseirialzer {
 ...
 return  РезультатОперацииBurkUPDATE.size();
        ...
        try{
            this.context=context;
                ...
                String  SQlOperInsert=  "INSERT INTO "+имяТаблицаAsync+" " +
                        " (   _id, cfo, month_tabels ,year_tabels,   date_update,  uuid,status_send,  user_update,    current_table   ) " +
                        " VALUES(?,?,?,?,?,?,?,?,?);";
    ...
    Integer ОбновлениеДанных(@NonNull Context context,
        ...
        try{
            this.context=context;
            // ОперацияInsert = Create_Database_СамаБАзаSQLite.insert(имяТаблицаAsync, null, ТекущийАдаптерДляВсего);
            String  SQlOperUpdate=  " UPDATE "+имяТаблицаAsync+"" +
                    " SET  _id=?, cfo=?, month_tabels =?,year_tabels=? ,  " +
                    " date_update=?,  uuid=?,status_send=?,  user_update=?,    current_table=?     WHERE  uuid=?  ;";
    ...
    private SQLiteStatement методGetSqliteStatementForInsert(@NonNull SQLiteDatabase Create_Database_СамаБАзаSQLite,
        ...
        try{
            ...
            sqLiteStatementInsert.bindLong(2, jsonNodeParentMAP.get("cfo").intValue());//"id""
            sqLiteStatementInsert.bindLong(3, jsonNodeParentMAP.get("month_tabels").intValue());//"id""
