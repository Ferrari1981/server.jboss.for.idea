...
public class СompanylJsonDeserializer extends JsonMainDeseirialzer {
 ...
 return  РезультатОперацииBurkUPDATE.size();
        ...
        try{
            this.context=context;
                ...
                String  SQlOperInsert=  " INSERT INTO "+имяТаблицаAsync+"  " +
                        "  (   _id,  name ,fullname,inn, kpp  , date_update ,user_update,uuid,current_table   )  " +
                        " VALUES (?,?,?,?,?,?,?,?,? ) ;";
    ...
    Integer ОбновлениеДанных(@NonNull Context context,
        ...
        try{
            ...
            String  SQlOperUpdate=  " UPDATE "+имяТаблицаAsync+" SET  _id=?, " +
                    ...
                    " date_update =?,user_update=?,uuid=?,current_table=?  "+
                    "   WHERE  uuid=?  ;";
    ...
    private SQLiteStatement методGetSqliteStatementForInsert(@NonNull SQLiteDatabase Create_Database_СамаБАзаSQLite,
        ...
        try{
            ...
            sqLiteStatementInsert.bindString(4, jsonNodeParentMAP.get("inn").asText().trim());//"date_update"
            sqLiteStatementInsert.bindString(5, jsonNodeParentMAP.get("kpp").asText().trim());//"date_update"
