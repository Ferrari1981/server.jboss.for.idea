...
/*        курсор     = resolver.query(uri,new String[]{"*"},"user_update=? AND cfo=? AND nomen_vesov=? AND status_send!=?",
    ...
    public  Long МетодВставкаНовойИлиВыбранойФотографииImageUpAndCreate(
            @NonNull CopyOnWriteArrayList<LinkedHashMap<Long, Bitmap>> copyOnWriteArrayListSuccessAddImages
        ...
        try{
            Flowable.fromIterable(copyOnWriteArrayListSuccessAddImages)
                    ...
                    .doOnNext(new Consumer<LinkedHashMap<Long, Bitmap>>() {
                        @Override
                        public void accept(LinkedHashMap<Long, Bitmap> integerBitmapLinkedHashMap) throws Throwable {


                            Long ImageРАзмерФотографии =
                                    integerBitmapLinkedHashMap.values().stream()
                            ...
                            if (ImageРАзмерФотографии>0) {
                                ...
                                Long  UUIDNewImage =    integerBitmapLinkedHashMap.keySet().stream().findFirst().get();
                             // 1  Long  UUIDNewImage = (Long) new Class_Generation_UUID(getApplicationContext()).МетодГенерацииUUID();
                                ...
                                contentValuesСозданиеНовогоМатериала = методВставкиИДекопресииНовойФотографии(integerBitmapLinkedHashMap, contentValuesСозданиеНовогоМатериала);
   ...
   return   arrayListРезультатНовойImageandUp.stream().mapToLong(v -> v).max().getAsLong();
    ...
    private ContentValues методВставкиИДекопресииНовойФотографии(
      ...
      //  bitmapNewImage.compress(Bitmap.CompressFormat.JPEG, 100, bytedataNewImage);
        bitmapNewImage.compress(Bitmap.CompressFormat.JPEG, 20, bytedataNewImage);
