...
public class FragmentMaretialNew extends Fragment implements CameraXInterface{
    ...
    private   CopyOnWriteArrayList<LinkedHashMap<Long,Bitmap>> copyOnWriteArrayListSuccessAddImages;
    ...
    private void методCAllBackWihtGenerratorNewImage() {
        ...
        fragmentManager.setFragmentResultListener("BackFromGeneratorImage", this, new FragmentResultListener() {
            ...
            public void onFragmentResult(@NonNull String requestKey, @NonNull Bundle result) {
                ...
                String bitmapPhotoNamegetUUID = result.getString("bitmapPhotoNamegetUUID");

                Long bitmapPhotoOnlyUUID = result.getLong("bitmapPhotoOnlyUUID");

                методCallsBackFromCameraX(bitmapPhoto,bitmapPhotoNamegetUUID,bitmapPhotoOnlyUUID);

                Log.d(getContext().getClass().getName(), "\n" + " CursorДляЦФО "
                        + CursorДляЦФО + " CursorДляОдногоМатериалаБышВесов " + CursorДляОдногоМатериалаБышВесов +
                        " CursorДляАвтомобиля " + CursorДляАвтомобиля + " CursorДляКонтрагента " +CursorДляКонтрагента
                        + " bitmapPhoto " +bitmapPhoto +  "bitmapPhotoNamegetUUID " +bitmapPhotoNamegetUUID  + " bitmapPhotoOnlyUUID " +bitmapPhotoOnlyUUID );
            } catch (Exception e) {
                e.printStackTrace();
                Log.e(getContext().getClass().getName(),
                        "Ошибка " + e + " Метод :" + Thread.currentThread().getStackTrace()[2].getMethodName() +
                        ...
                        try{
                            ...
                            Intent dataUpImage = result.getData();
                                ...
                                String NameNewPhotosCamerax=UUID.toString() +".jpg";
                                  bundleUpImage.putString( "bitmapPhotoNamegetUUID",NameNewPhotosCamerax);
                                  bundleUpImage.putLong( "bitmapPhotoOnlyUUID",UUID);
                                dataUpImage.putExtras(bundleUpImage);
...
void методCallsBackFromCameraX(@NonNull  Bitmap bitmapNewCompleteImage,@NonNull String bitmapPhotoNamegetUUID,@NonNull   Long bitmapPhotoOnlyUUID){
    try{
            if (bitmapNewCompleteImage != null){
                ...
                dataCreateNewImage.putExtra("bitmapPhotoNamegetUUID",bitmapPhotoNamegetUUID);
                dataCreateNewImage.putExtra("bitmapPhotoOnlyUUID",bitmapPhotoOnlyUUID);
                            ...
                            if(IDДляВставкиГруппыматериалов>0 && IDДляВставкиОдногоматериала>0
                                ...
                                if (ХэшРезультататСозданияСозданиеНовогоМатериала>0) {
                                    ...
                                    if (copyOnWriteArrayListSuccessAddImages.size()>0) {
                                        ...
                                        Log.d(this.getClass().getName(), " ХэшРезультататСозданияНовыхИлиВыбранныхФотографий " +ХэшРезультататСозданияНовыхИлиВыбранныхФотографий );
